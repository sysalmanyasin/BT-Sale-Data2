...
//   manager       : merged per-month, per-category (Salary/Generic/Expense/Credit/Incentive) —
//                   remote wins on an exact month collision within a category, but any month
//                   only one side has touched is preserved either way
//   petty         : remote wins per month (each month is its own localStorage key already)
//   custom        : merged per-section AND per-month within each section — remote wins on an
//                   exact section+month collision, local-only sections/months are kept
// Returns counts so callers can log what changed.

// Merge two "month-keyed" plain objects ({ "July 2026": <data>, ... }).
// Remote wins on an exact month-key collision; months only one side has are kept.
function _mergeMonthMap(localMap, remoteMap) {
  return Object.assign({}, localMap || {}, remoteMap || {});
}

const MGR_CATEGORIES = ['salary', 'generic', 'expense', 'credit', 'incentive'];
...
function mergeIncomingData(data) {
  ...
  // Manager blob (Salary, Generic, Expense/Patty, Staff Credits, Incentives) —
  // merge each category at the month-key level so unrelated months/categories
  // from either side never get wiped out wholesale.
  if (data.manager) {
    const cur = JSON.parse(localStorage.getItem(MGR_KEY) || '{}');
    const merged = { ...cur };
    MGR_CATEGORIES.forEach(cat => {
      if (cur[cat] || data.manager[cat]) merged[cat] = _mergeMonthMap(cur[cat], data.manager[cat]);
    });
    // Any other/future top-level keys present remotely but missing locally are adopted as-is
    Object.keys(data.manager).forEach(k => { if (!(k in merged)) merged[k] = data.manager[k]; });
  ...
  // Custom sections — merge per section, and within each section, per month.
  if (data.custom) {
    const localCus = JSON.parse(localStorage.getItem(CSEC_KEY) || '{}');
    const merged = { ...localCus };
    Object.entries(data.custom).forEach(([sid, remoteSec]) => {
      if (!merged[sid]) { merged[sid] = remoteSec; return; }
      merged[sid] = {
        name:   remoteSec.name  || merged[sid].name,
        emoji:  remoteSec.emoji || merged[sid].emoji,
        months: _mergeMonthMap(merged[sid].months, remoteSec.months)
      };
    });
