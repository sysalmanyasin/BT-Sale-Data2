...
function _nsSpRibbonHTML(tab) {
  ...
  if (tab === 'data') {
    return `
      ...
      <button class="ns-rb-btn" onclick="_nsSpImportXLSX()">⬆ Import Workbook</button>
      <div class="ns-rb-sep"></div>
...
// Reads EVERY worksheet tab in the picked .xlsx/.xls file — not just the
// first — and lands each one as its own grid inside one brand-new file
// (named after the source file), same workbook storage as everything
// else in Sheets. Previous behavior (overwrite the currently active
// grid with only the first tab) is gone: a multi-tab supplier price
// list or export now arrives in one shot instead of one sheet at a
// time, and never clobbers whatever the user already had open.
function _nsSpImportXLSX() {
  ...
  inp.addEventListener('change', e => {
    ...
    reader.onload = ev => {
      const srcWb = XLSX.read(new Uint8Array(ev.target.result), { type: 'array' });
      const sheetNames = srcWb.SheetNames || [];
      if (!sheetNames.length) { if (typeof toast === 'function') toast('⚠ No worksheets found in that file.', 'w'); return; }
      ...
      sheetNames.forEach(name => {
        ...
        const g = _nsDefaultGrid(name.slice(0, 31) || 'Sheet');
        rows.forEach((row, r) => {
      ...
      wb.files[fid] = {
        id: fid, name: fileName, category: 'Imported',
        createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
        grids, activeSheet: firstGid,
      };
      wb.activeFileId = fid;
      _nsWBSaveRaw(wb);
      _spState.activeSheet = firstGid;
      _nsSpRenderRibbon();
      _nsSFRefreshOpenViews();
      _nsRenderPanel();
      if (typeof toast === 'function') toast(`✅ Imported "${fileName}" — ${sheetNames.length} sheet${sheetNames.length===1?'':'s'}, ${totalRows} rows total`);
    };
...
/* ══════════════════════════════════════════════════════════════════════
   DATA PANEL  —  live, read-only view across every domain's own tables
   (Sales/Manager/Inventory), plus 📥 Send to Sheets to materialize
   whatever's showing into a real, editable grid in the Sheets tab.

   Registry-driven: _NS_DOMAINS groups source ids under a domain picker,
   _NS_SOURCE_META has each source's label/icon/empty-state/hint text,
   _NS_SOURCE_COLS has each source's [key, type, label] columns (type is
   'text' | 'num' | 'money', used by _nsCell for display formatting and
   by XLSX export / Send to Sheets for what to write). _nsSourceRows(id)
   is the single place that knows how to pull each table's actual rows —
   render, export, and Send to Sheets all call through it, so there's
   exactly one definition of what each table's rows are.

   Manager sources (salary/generic/credit/petty/incentive) reflect
   whichever month is currently loaded in that Manager tab this session
   — same "current working copy" every other consumer of these live
   bindings already relies on (ai-bridge.js's AI-assistant commands,
   Quick Add, etc.) — NOT a full-history export. Credit Ledger and
   Unmatched are the exception: _crdData_cur re-syncs on every load and
   unmatchedLoad() reads Repository fresh each call, so both are always
   current regardless of what's been opened this session.
══════════════════════════════════════════════════════════════════════ */
const _NS_DOMAINS = [
...
function _nsSetDataDomain(v) {
  ...
  _nsRenderPanel();
} // see _nsSetMgsSearch for why this indirection is needed
...
// full set up to 5000 rows) into a brand-new, fully editable grid in
// the Sheets tab — a real file in the same workbook storage every other
// sheet already uses (bt_sheet_workbooks_v1 → Repository → synced),
// so from here it's just a normal spreadsheet: edit, formulas, export,
