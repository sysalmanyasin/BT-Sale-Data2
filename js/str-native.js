...
function fmtQty(v) { return v === null || v === undefined ? '' : (Number(v) || 0).toLocaleString('en-PK'); }
function fmtMoney(v) { return 'Rs ' + (Number(v) || 0).toLocaleString('en-PK', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }

// ── Derived 3-stage lifecycle — str_status itself is only Open/Close;
// the Awaited/Dispatched/Received stages the report is filtered by come
...
const STAGE_LABEL = { awaited: 'Awaited', dispatched: 'Dispatched', received: 'Received' };
const STAGE_BADGE_CLASS = { awaited: 'str-badge-amber', dispatched: 'str-badge-blue', received: 'str-badge-green' };
...
// receive branch.
function isDispatchedFromBT(h) { return h.direction === 'out'; }
function isReceivedAtBT(h) { return h.direction === 'in'; }
...
function _lineItemsForStr(data, strId) {
  return data.lineItems.filter(li => li.strId === strId);
}

function _visibleHeaders(data) {
  let list = data.headers;
  if (strState.branchFilter === 'dispatched') list = list.filter(isDispatchedFromBT);
  else if (strState.branchFilter === 'received') list = list.filter(isReceivedAtBT);
  if (strState.stageFilter !== 'all') list = list.filter(h => strStage(h) === strState.stageFilter);
  if (strState.dateFrom) list = list.filter(h => h.strDate && h.strDate >= strState.dateFrom);
  if (strState.dateTo) list = list.filter(h => h.strDate && h.strDate <= strState.dateTo);
  const q = strState.search.toLowerCase().trim();
  if (q) {
    list = list.filter(h =>
...
function _rowHtml(h, itemCount) {
  const stage = strStage(h);
  return `<tr class="str-row" onclick="strOpenDetail(${h.strId})">
    <td>
      <div style="font-size:13px;font-weight:700;color:var(--text)">${esc(h.strNumber || '—')}</div>
      <div style="font-size:10px;color:var(--muted);margin-top:2px">Master ${esc(h.masterStrId || '—')}</div>
    </td>
    <td style="font-size:12px;color:var(--text)">${fmtDate(h.strDate)}</td>
    <td style="font-size:11.5px;color:var(--t2)">${esc(h.dispatchBranch || '—')} → ${esc(h.receiveBranch || '—')}</td>
    <td><span class="str-badge ${STAGE_BADGE_CLASS[stage]}">${STAGE_LABEL[stage]}</span></td>
    <td style="font-size:11.5px;color:var(--muted)">${esc(h.issuedBy || '—')}</td>
...
function renderStrPage() {
  ...
  const dispatchedCount = data.headers.filter(isDispatchedFromBT).length;
  const receivedCount = data.headers.filter(isReceivedAtBT).length;
  ...
  tbody.innerHTML = visible.map(h => _rowHtml(h, _lineItemsForStr(data, h.strId).length)).join('');
}
...
// ── Detail modal ─────────────────────────────────────────────────────
// Groups line items by supplier (joined via data.supplierByCode) and
// sorts each group by product code ascending — see this file's header
// note on why, and why that join has to happen client-side.
function _groupedLineItems(data, strId) {
  const items = _lineItemsForStr(data, strId);
  ...
  items.forEach(li => {
    ...
    bySupplier.get(supplier).push(li);
  });
...
function _diffQty(li) {
  if (li.dispatchQty === null || li.receiveQty === null) return null;
  return li.dispatchQty - li.receiveQty;
}

function _detailLineRowHtml(li, sr) {
  const diff = _diffQty(li);
  ...
  return `<tr class="str-detail-row">
    ...
    <td>
      <span style="color:var(--muted);font-size:11px">[${esc(li.productCode || '—')}]</span>
      <span style="font-weight:600">${esc(li.productName)}</span>
    </td>
    <td style="text-align:right">${fmtMoney(li.productPrice)}</td>
    <td style="text-align:right">${fmtQty(li.strQty)}</td>
    <td style="text-align:right">${fmtQty(li.dispatchQty)}</td>
    <td style="text-align:right">${fmtQty(li.receiveQty)}</td>
    <td style="text-align:right">${diffHtml}</td>
  </tr>`;
}

function _detailGroupHtml(group, srStart) {
  const subStr = group.rows.reduce((s, r) => s + (r.strQty || 0), 0);
  const subDisp = group.rows.reduce((s, r) => s + (r.dispatchQty || 0), 0);
  const rowsHtml = group.rows.map((li, i) => _detailLineRowHtml(li, srStart + i)).join('');
  return `
    <tr class="str-detail-supplier-row"><td colspan="7">${esc(group.supplier)}</td></tr>
    ${rowsHtml}
    <tr class="str-detail-subtotal-row">
      <td colspan="3">Subtotal — ${group.rows.length} item(s)</td>
      <td style="text-align:right">${fmtQty(subStr)}</td>
      <td style="text-align:right">${fmtQty(subDisp)}</td>
      <td></td><td></td>
    </tr>`;
}

function _detailHeaderFieldsHtml(h) {
  const field = (label, val) => `<div class="str-detail-field"><span class="str-detail-field-label">${label}</span><span class="str-detail-field-val">${esc(val || '—')}</span></div>`;
  return `
...
function renderStrDetail() {
  ...
  if (!h) { body.innerHTML = `<div style="font-size:13px;color:var(--muted);padding:20px 0;text-align:center">Could not load this STR — it may have rolled out of the 7-day sync window.</div>`; return; }

  const groups = _groupedLineItems(data, h.strId);
  let sr = 1;
  const groupsHtml = groups.map(g => { const html = _detailGroupHtml(g, sr); sr += g.rows.length; return html; }).join('');
  const grandStr = groups.reduce((s, g) => s + g.rows.reduce((s2, r) => s2 + (r.strQty || 0), 0), 0);
  const grandDisp = groups.reduce((s, g) => s + g.rows.reduce((s2, r) => s2 + (r.dispatchQty || 0), 0), 0);
    ...
    <div class="str-detail-table-wrap">
      <table class="str-detail-table">
        <thead><tr>
          ...
          <th style="text-align:right">STR Qty</th>
        ...
        <tfoot><tr class="str-detail-grand-row">
          ...
          <td style="text-align:right">${fmtQty(grandStr)}</td>
          <td style="text-align:right">${fmtQty(grandDisp)}</td>
          <td></td><td></td>
        </tr></tfoot>
      </table>
    </div>
    ${h.comments ? `<div class="str-detail-comments"><strong>Comments:</strong> ${esc(h.comments)}</div>` : ''}
  `;
}
...
function strPrintDetail() {
  ...
  const fieldRow = (label, val) => `<td style="padding:3px 10px 3px 0;font-size:11px;color:#555;white-space:nowrap">${label}</td><td style="padding:3px 16px 3px 0;font-size:12px;font-weight:600">${esc(val || '—')}</td>`;

  const groupsHtml = groups.map(g => {
    const subStr = g.rows.reduce((s, r) => s + (r.strQty || 0), 0);
    const subDisp = g.rows.reduce((s, r) => s + (r.dispatchQty || 0), 0);
    const rows = g.rows.map(li => {
      const diff = _diffQty(li);
      const cur = sr++;
      return `<tr>
        <td style="padding:4px 6px;border-bottom:1px solid #eee;font-size:11px;text-align:center">${cur}</td>
        <td style="padding:4px 6px;border-bottom:1px solid #eee;font-size:11px">[${esc(li.productCode)}] ${esc(li.productName)}</td>
        <td style="padding:4px 6px;border-bottom:1px solid #eee;font-size:11px;text-align:right">${fmtMoney(li.productPrice)}</td>
        <td style="padding:4px 6px;border-bottom:1px solid #eee;font-size:11px;text-align:right">${fmtQty(li.strQty)}</td>
        <td style="padding:4px 6px;border-bottom:1px solid #eee;font-size:11px;text-align:right">${fmtQty(li.dispatchQty)}</td>
        <td style="padding:4px 6px;border-bottom:1px solid #eee;font-size:11px;text-align:right">${fmtQty(li.receiveQty)}</td>
        <td style="padding:4px 6px;border-bottom:1px solid #eee;font-size:11px;text-align:right">${diff === null ? '' : (diff > 0 ? '+' : '') + diff}</td>
      </tr>`;
    }).join('');
    return `<tr><td colspan="7" style="padding:8px 6px 4px;font-size:11.5px;font-weight:800;background:#f3f4f6">${esc(g.supplier)}</td></tr>
      ${rows}
      <tr><td colspan="3" style="padding:4px 6px;font-size:11px;font-weight:700;text-align:right">Subtotal (${g.rows.length} item(s))</td>
        <td style="padding:4px 6px;font-size:11px;font-weight:700;text-align:right">${fmtQty(subStr)}</td>
        <td style="padding:4px 6px;font-size:11px;font-weight:700;text-align:right">${fmtQty(subDisp)}</td>
        <td></td><td></td></tr>`;
  }).join('');
  ...
  const html = `<div style="max-width:900px;margin:0 auto;font-family:Arial,sans-serif;color:#111">
    <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #111;padding-bottom:10px;margin-bottom:10px">
      ...
      <div style="text-align:right;font-size:12px"><strong>STR #${esc(h.strNumber)}</strong><br><span style="color:#666;font-size:11px">Master ${esc(h.masterStrId)}</span></div>
    </div>
    <table style="width:100%;border-collapse:collapse;margin-bottom:12px">
      <tr>${fieldRow('From', h.dispatchBranch)}${fieldRow('To', h.receiveBranch)}${fieldRow('Media', h.media)}</tr>
      <tr>${fieldRow('STR Date', fmtDate(h.strDate))}${fieldRow('STR Status', h.strStatus)}${fieldRow('Ref #', h.refNo)}</tr>
      <tr>${fieldRow('Dispatched Date', fmtDate(h.dispatchedDate))}${fieldRow('Dispatch Status', h.dispatchStatus)}${fieldRow('Dispatched By', h.dispatchedBy)}</tr>
      <tr>${fieldRow('Receive Date', fmtDate(h.receiveDate))}${fieldRow('Receive Status', h.receiveStatus)}${fieldRow('Received By', h.receivedBy)}</tr>
    ...
    <table style="width:100%;border-collapse:collapse">
      ...
      <tfoot><tr style="border-top:2px solid #111">
        ...
        <td style="padding:6px;font-size:12px;font-weight:800;text-align:right">${fmtQty(grandStr)}</td>
        <td style="padding:6px;font-size:12px;font-weight:800;text-align:right">${fmtQty(grandDisp)}</td>
        <td></td><td></td>
      </tr></tfoot>
    </table>
    ${h.comments ? `<div style="margin-top:10px;font-size:11px;color:#444"><strong>Comments:</strong> ${esc(h.comments)}</div>` : ''}
  </div>`;
