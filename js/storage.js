...
async function idbLoadData() {
  try {
    ...
    mArr.forEach(m => { if (!MONTHLY.find(x => x.Month_Year === m.Month_Year)) MONTHLY.push(m); });
    dArr.forEach(d => { if (!DAILY.find(x => x.Date === d.Date && x.Month_Year === d.Month_Year)) DAILY.push(d); });
    return true;
