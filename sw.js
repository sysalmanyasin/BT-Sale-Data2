...
const CACHE_NAME = 'bt-sales-v10.87'; // v10.87: Cover's .cover-group (section cards, e.g. SALES/STR REPORT) and .cover-hero-card (KPI hero cards inside them) had a background gradient that faded to plain white within the first 38-60% of the card, so most of the card read as flat white with just a colored left border/icon. Now both keep a full domain-tinted wash across the whole card (color-mix blend instead of fading to --surface) plus a soft radial glow, and .cover-group gained a domain-tinted border on all four sides (was plain gray) so the rounded corners actually read instead of blending into the page background.
