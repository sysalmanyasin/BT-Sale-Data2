...
const CACHE_NAME = 'bt-sales-v11.06'; // v11.06: Emergency Billing — percentage discount mode (Rs./% toggle next to the discount field, plus 1%/2%/3%/4%/5% quick-preset chips shown in % mode; the rupee-equivalent is shown underneath as "= Rs. X.XX"). calcTotals() now resolves either mode down to a rupee amount before anything downstream (receipt, checkout RPC, held bills) ever sees it, so nothing else had to change. Held bills always freeze/restore the discount as a flat rupee figure regardless of which mode it was entered in (holdBill()/recallHeld()). Also: F9 edit mode can now jump straight into a row's qty field to overwrite it — pressing Enter opens it with the value selected, or a digit key opens it and starts typing that digit immediately; Enter inside the field commits, Escape cancels and reverts (onCartBodyKeydown(), new). And the native up/down spinner arrows on the qty field (and the refund modal's matching qty field) are hidden — redundant next to the existing custom −/+ buttons beside them. Bumped css/emergency-billing.css and js/emergency-billing-native.js to ?v=20260925b (index.html) plus this CACHE_NAME.
// v11.05: Emergency Billing — added a tab bar (Billing / History / Settings) to the existing break-glass till page. History: date presets (Today/Yesterday/7 Days/This Month/All), invoice #, product, payment method and customer/staff filters over emergency_invoices + a new two-step product lookup against emergency_invoice_items (js/emergency-billing-bridge.js's new searchInvoiceNumbersByProduct()); clicking a result "loads" the saved bill in a read-only detail modal with Reprint (routes back through the existing showReceipt()/print flow, now date-stamped with the ORIGINAL sale's billed_at and marked "REPRINT") and, for a non-refund invoice, a "Refund This" shortcut into the existing Refund modal. Settings: Branch Identity, Business Name, Receipt Customization (header/footer note, 58mm/80mm width, currency symbol, address/phone toggles) and Billing Settings (default payment method, low-stock threshold, require-staff-name, auto-print, confirm-before-clear, round-net-to-nearest-rupee) — all localStorage-only per-device config (eb_settings_v1), same reasoning as this page's existing held-bills/cart keys; applied live to cur()/doSearch()'s low-stock badge/calcTotals()/showReceipt(). Also added this page's own css/emergency-billing.css + js/emergency-billing-bridge.js + js/emergency-billing-native.js to APP_SHELL below — they existed and were already linked from index.html, but had never actually been added to precache. Bumped all three to ?v=20260925a (index.html) plus this CACHE_NAME.
// v11.04: Staff Attendance — new Manager > Attendance tab (js/attendance-bridge.js reads/writes attendance_events/attendance_devices/attendance_locations in the main Supabase project, js/manager-attendance.js renders Today/Monthly/Raw Log/Manual Entry views), css/attendance.css. Companion native Android app (geofence + QR check-in/out) lives outside this codebase, built the same way as android-widget/. See supabase/migrations/*_attendance.sql for schema + RLS. Previously v11.03: IC Herald — new collapsible daily "newspaper" on Cover (js/herald/herald-engine.js + herald-page.js, css/herald.css), replacing the old #cover-attention-strip chip row (js/cover-dashboard.js's _needsAttention()/_renderAttentionStrip() removed, superseded). herald-engine.js is a pure Floor-3-style insight catalog spanning every desk — Sales (reuses Analytics.computeInsightCandidates()/getDashboardKPIs() for weekday/MoM/YoY/target-pace/forecast, plus new streak/record detection and a Cash-to-Deposit reconciliation mirroring cash-deposit-report.js's own formula), Manager (staff credit via _crdData/_crdNet, salary via _salRows/_salNet, petty via _pettyTotalForMonth), Closing (ClosingBridge shift/credit-ledger summary), Inventory (computeInventoryBuckets() from shared/summary-calc.js + Reorder Report), Audit (AuditBridge engagement summary), plus a Cross-Desk correlation and a Milestone desk for records/streaks — all folding in aimRulesCheckAll()'s existing rule alerts too. A scoring engine (tier weight × 14-day novelty decay, minimum-spacing floor per tier, desk-rotation-aware lead pick, per-insight phrase banks) picks one "edition" per calendar day, cached (bt_herald_edition_cache) so it doesn't reshuffle on intraday re-renders; logs to bt_herald_shown_log (pruned to 90 days) and bt_herald_lead_desk_history so the same headline/desk doesn't dominate every morning. Collapsed by default (bt_herald_collapsed, UI-local), expands to a full front page with a Print Edition button (routes through Print.render(), same engine every other report uses) and a manual "Later Edition" refresh. New: js/herald/herald-engine.js, js/herald/herald-page.js, css/herald.css (index.html, this file's APP_SHELL). index.html: #cover-attention-strip renamed to #cover-herald.
// v11.02: BT Game Play follow-up — (1) Weekdays tab now has a "Last N of each weekday" dropdown (4/8/12/26/52, was a fixed 8); (2) Same-month tab now has both a month dropdown (any month, was locked to the current calendar month) and a "last N years" dropdown (3/5/7/10/all, was a fixed 5); (3) new manual forecast override on the Race the Forecast card — an optional "your own ₨/day for the remaining days" input; when set, a second accent-colored ● marker shows where that plan lands next to the algorithm's own 🏃 pace marker, and the bar's on-track color follows the plan instead of the algorithm once one's entered. Live-typing updates only reposition the ● marker (no rebuild, so the input never loses focus mid-keystroke); the rebuild/rescale happens on blur/Enter. Persisted per Month_Year via Repository/Actions.saveFeatureData (UI-state door, same one dashboard-insights.js's own briefing/diff-badge state already uses) — nothing written to DAILY/MONTHLY. All still js/dashboard-race.js, no other file touched. Bumped js/dashboard-race.js to ?v=20260913b (index.html) plus this CACHE_NAME.
// v11.01: BT Game Play — two new playful Dashboard widgets, both pure renderers over numbers Analytics already computes (no new business data/storage). (1) "Race the Forecast" (js/dashboard-race.js, new file): a single bar racing MTD actual against the target line and this month's forecast-at-current-pace marker, reusing Analytics.getDashboardKPIs()'s own latTgt/latAct/forecastTotal — same figures already behind the "Forecast vs Target" KPI card and dashboard-insights.js's Target Pace card, just drawn as a race. (2) "Bar Race": replayable animated bar race, two tabs — Weekdays (last-8-occurrences average per weekday, all 7 at once; new Analytics.getRecentWeekdayAverages(), generalizing computeInsightCandidates()'s single-weekday check which only ever looked at today's) and "<Month> × years" (e.g. every August for the last 5 years; new Analytics.getSameMonthAcrossYears()). Both new Analytics functions are Floor 3 (js/analytics.js), added alongside getTargetPaceForMonth/computeInsightCandidates. Wired into buildDashboard()'s existing render sequence (js/dashboard.js) behind a typeof guard, same convention as buildSalesHeatmap/buildDashboardInsights. New: js/dashboard-race.js (index.html, this file's APP_SHELL). Bumped js/analytics.js to ?v=20260913a, js/dashboard.js to ?v=20260913a (index.html) plus this CACHE_NAME.
// v11.00: Dashboard — new "Sales Calendar" GitHub-style year heatmap (js/dashboard-heatmap.js, new file) sitting under Best/Worst Month per Year. One cell per calendar day of DAILY.TOTAL, shaded by quartile within that year's own sale days (not a fixed rupee cutoff), Sun-start week columns same as GitHub's own graph, month labels along the top. Own year-chip toggle, independent of the #dash-year select (same reasoning as the Monthly Total chart's own preset/compare state below it — a calendar is inherently annual, while Top 10 Days/Day-of-Week/Best-Worst next to it are deliberately all-time). Tapping a filled day opens that day's existing detail modal (openDayModal, reports.js) — no new modal. Wired into buildDashboard()'s existing render sequence (js/dashboard.js) behind a typeof guard, same convention as buildDashboardInsights. New: js/dashboard-heatmap.js (index.html, this file's APP_SHELL). v10.99: FIX — deleted ledger rows (Misc Sections, Expense/Petty, Jazz Cash Daily Ledger), deleted Misc Section types, and deleted Jazz Cash Balance Tally accounts/snapshots were coming back after the next refresh/sync. Root cause: js/supabase.js's mergeIncomingData() merges every one of those lists as a plain union-by-id ("remote wins on pull, local fills gaps from remote on push") with no concept of a delete — an id missing locally just looked like "not yet synced" and got silently refilled from Supabase, including on push itself (_doPush() pulls remote in to "fill gaps" before re-uploading, so a device could resurrect its own just-deleted row right before pushing). Fixed with real tombstones: new js/sync-tombstones.js records "this id was deleted at time T" locally, travels in every push payload, and is checked by every affected merge block before an id is kept. js/ledger-store.js's _removeEntry()/deleteCustomLedgerType() and js/jazz-cash.js's _tallyRemoveAcc()/_tallyDeleteSnap() now record tombstones at the moment of deletion. Bumped js/sync-tombstones.js (new), js/ledger-store.js to ?v=20260901a, js/supabase.js to ?v=20260901a, js/jazz-cash.js to ?v=20260901a (index.html) plus this CACHE_NAME.
// v10.98: Manager Dashboard — Staff Credit names in the Credit Details → Staff Credit card are now clickable (js/dashboard.js), opening that employee's Staff Card the same way a name in Staff Registry does (openStaffCard, resolved by matching name against STAFF). New staffDetailRows() renderer used only for that card; Jazz Cash and Misc Section cards are unchanged. Bumped js/dashboard.js's query string to ?v=20260831a (index.html) plus this CACHE_NAME. // v10.97: Manager Dashboard — removed the redundant standalone add-form above the Jazz Cash and Expense ledger tables (js/ledger-page.js). Both already have a "Quick Add" panel above them that writes into the same ledger via the same LedgerActions.addEntry() door, so renderLedgerView() now skips that form (and its submit wiring) specifically for ledgerType 'jazzcash' and 'expense'. Every other ledger view (custom Other Sections) is unchanged. Bumped js/ledger-page.js's query string to ?v=20260831b (index.html) plus this CACHE_NAME. // v10.96: DIFF Report — day-wise expandable sub-report per month (js/diff-report.js). Clicking a month row now expands a nested day-by-day table (Date / Total Sale / COMP SALE / Difference / Running Total) beneath it, reusing the existing .mon-group/.mon-chevron expand pattern from the Daily Data page (css/components.css) rather than inventing new styles. Per-day running total resets to 0 at the start of each month's sub-report (the top-level monthly running total is unaffected). Bumped js/diff-report.js's query string to ?v=20260831a (index.html) plus this CACHE_NAME so the service worker's own precache picks up the real content too. v10.95: CACHE-BUSTING FIX — every CSS edit from today's branding pass
...
const APP_SHELL = [
  ...
  './js/herald/herald-engine.js',
  './js/herald/herald-page.js',
  './js/status-bar.js',
  './js/record-day.js',
  './js/staff-notes.js',
  ...
  './js/fields.js',
  './js/drive.js',
  './js/emergency-billing-bridge.js',
  './js/emergency-billing-native.js',
