...
const CACHE_NAME = 'bt-sales-v10.18'; // v10.18: Staff Payslip — new "🧾 Payslip" tab inside the Staff Card (js/manager-payslip.js, new file, added to APP_SHELL below + a new <script type="module"> tag in index.html right after manager-credit.js). Per-employee, per-month slip combining Salary sheet (HO Salary/Advance/Generic Add-on/Net) + Credit Ledger (Prev Balance/entries/Salary Paid/Less Generic/Net) into one view, with 🖨 Print (funnels through the existing Print.render() engine, same as every other report) and 📱 Send via WhatsApp (opens wa.me with the slip pre-filled as text — no Business API template needed, unlike the automated daily briefing in supabase/functions/). index.html also got the new tab button + #sc-panel-payslip markup, and js/manager-staff.js's switchStaffCardTab() now knows about the new panel. Same "silently missing offline" class of bug as every earlier entry below if manager-payslip.js weren't added to APP_SHELL — version bumped so already-installed clients actually fetch it instead of 404ing when the new tab is clicked.
