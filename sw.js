...
const CACHE_NAME = 'bt-sales-v10.88'; // v10.88: All Sections drawer (long-press Cover / ☰) redesign — colorful pass. Gradient brand header instead of flat modal chrome; top-level groups now render as domain-tinted cards (colored left border + colored icon chip + matching hover/chevron tint), keyed off each domain's own accent token (sales=accent, manager=mgrblue, notesheets=green, closing=teal, audit=amber, inventory=pink, str=red, reports=purple, tools=accent-dk) so it visually matches that domain's own dashboard instead of a flat neutral list. Search box restyled as a rounded pill with a leading icon and accent focus glow. Scoped entirely to #sectionsbg/.sections-*/.gs-* — see css/nav.css's "Colorful design pass" block and js/nav-sections.js's data-domain/data-section attributes.
