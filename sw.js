...
const CACHE_NAME = 'bt-sales-v10.77'; // v10.77: STR "Report" tab gets a "Hide 0-Dispatch" toggle button (js/str-report-native.js) — persisted per-device (bt_str_report_hidezero_v1), drops any line still at Dispatch Qty = 0 from a STR's supplier groups, and any STR left with zero groups after that drops off the page entirely (same "empty STR disappears" behavior the Zero Dispatch tab uses, inverted). Applies on-screen and in the printed version. Refactored _strBlockHtml/_branchGroupHtml/_groupedByBranch to work off pre-filtered {h, groups} entries (new _visibleGroups/_visibleEntries helpers) instead of re-deriving groupedLineItems per call. New css/str-report.css rule: .str-rpt-btn-active.
