...
const CACHE_NAME = 'bt-sales-v10.22'; // v10.22: Cash Deposit thermal/PDF print — the last two commits changed js/print.js twice (moving the html2canvas capture host from off-screen positioning to opacity:0-in-viewport, then fixing the actual root cause: variables.css's site-wide overflow-x:hidden on html/body was clipping the wider-than-viewport capture host at the phone's real screen width, dropping the header/value column/footer off the printed receipt) but never bumped this version — so already-installed clients kept serving the stale cached print.js through both fixes, which is why the printed output looked byte-for-byte unchanged after a hard refresh. print.js was already correctly listed in APP_SHELL below since v10.1; the file path never needed to change, only this version number, to force already-installed clients to actually refetch its new content instead of holding the old cached copy indefinitely.
