...
begin
    ...
    if not exists (
        select 1 from emergency_invoices ei
         where ei.invoice_number = p_original_invoice_number and ei.is_refund = false
    ) then
    ...
    perform 1 from emergency_invoices ei where ei.invoice_number = p_original_invoice_number for update;
    ...
    loop
        ...
        select coalesce(eii.qty, 0) into v_orig_qty
          from emergency_invoice_items eii
         where eii.invoice_number = p_original_invoice_number and eii.product_code = v_code;
